...
4. **Piecewise temporary impact `$g_t(x)$`**  
   ...
   Flat marginal cost within displayed depth; concave tail beyond it.

5. **Optimal allocation `$x_t$` (KKT + bisection)**  
   Minimize `$\sum_t x_t\,g_t(x_t)$` s.t. `$\sum_t x_t=S,\; x_t\ge 0$`.  
   Stationarity: `$\dfrac{d}{dx}\big[x\,g_t(x)\big]=\lambda$`. Invert:

   $$
   x_t(\lambda)=
   \begin{cases}
   D_t, & \lambda\le c,\\[4pt]
   \max\!\Big\{D_t,\; \Big(\dfrac{\lambda}{a_t(p+1)}\Big)^{1/p}\Big\}, & \lambda>c.
   \end{cases}
   $$

   Use bisection on `$\lambda$` so that `$\sum_t x_t(\lambda)=S$`.
